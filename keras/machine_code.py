...
legal_moves_c = cfunc_caller.generate_function(code_legal_moves, ctypes.c_int8, ctypes.c_char_p, ctypes.c_char, ctypes.c_char_p)
...
zobrist_c = cfunc_caller.generate_function(code_zobrist, ctypes.c_ulonglong, ctypes.c_char_p, ctypes.POINTER(ctypes.c_ulonglong), ctypes.POINTER(ctypes.c_ulonglong), ctypes.c_ulonglong)
...
def legal_moves(board, color):
    retlen = legal_moves_c(array_pointer(board), color, buf_pointer)
    return [buf[i] for i in range(retlen)]

def frontier_squares(board, color):
    return fsquares_c(array_pointer(board), color)

def weight_matrix(board, color):
    return wmatrix_c(array_pointer(board), color, weight_pointer)

def count_colors(board, color):
    return ccount_c(array_pointer(board), color)
