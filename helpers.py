...
# After some machine learning:
SQUARE_WEIGHTS_BAD = [
    72, -11, 10, 5, -5, 16, -16, 72,
    -5, -35, -1, -12, -9, -4, -31, -12,
    10, 1, -4, 2, 0, -1, -9, 18,
    1, -2, 4, 0, 0, 0, -3, -5,
    -12, 0, 2, 0, 0, 6, -2, 3,
...
class Node:
    def __init__(self, board, prev_player):
        self.board = board
        self.moving_plr = cinv(prev_player)
        self.ptr = machine_code.array_pointer(board)
        self.legal_moves = legal_moves(self)
        actual_color = next_player(self, prev_player)
        if actual_color != cinv(prev_player):
            if actual_color is not None:
                self.moving_plr = actual_color
                self.legal_moves = legal_moves(self)
            else: # it is none
                self.legal_moves = []
                self.moving_plr = actual_color
            # self.next_player = next_player(self)
...
def next_player(node, prev_player):
    p1_movs = node.legal_moves
    if len(p1_movs) == 0:
        node.moving_plr = cinv(node.moving_plr)
        p2_movs = legal_moves(node)
        node.moving_plr = cinv(node.moving_plr)
        if len(p2_movs) == 0:
            return None
        return cinv(node.moving_plr)
    return node.moving_plr
