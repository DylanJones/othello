        ...
        if (board[i] == 0) {
            for (int rS = -1; rS <= 1; rS++) {
                for (int cS = -1; cS <= 1; cS++) {
                    ...
                    int tempC = c + cS;
                    bool iterated = false;
                    while (0 <= tempR && tempR < 8 && tempC >= 0 && tempC < 8 && board[tempR*8+tempC] == ((color == 1) ? 2 : 1)) {
                        iterated = true;
                        tempR += rS;
                        tempC += cS;
                    }
                    if (tempR < 0 || tempR >= 8 || tempC < 0 || tempC >= 8 || !iterated) {
                        continue;
                    }
                    if (board[tempR*8+tempC] == color) {
                        if ((numMoves == 0 || output[numMoves-1] != i) && numMoves < BOARD_SIZE) {
                            output[numMoves] = i;
                            //output[numMoves] = i;
                            numMoves++;
                        }
