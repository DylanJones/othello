...
def main():
    ...
    player = WHITE
    while next_player(board, player) is not None:
        ...
        if actual_move not in legal_moves(board, player):
            raise RuntimeError(
        ...
        print_board(board)
    print("Black won!" if count_colors(board, BLACK) > 0 else "Black lost!")
...
p1 = run_our_strategy
p2 = run_their_strategy
# p2 = run_random_strategy

their_strat = "nomemory" if len(sys.argv) <= 1 else sys.argv[1]
